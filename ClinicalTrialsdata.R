...
rm(list = ls())
#Set TS5 folder on the desktop as the working directory
setwd("C:/Users/nicki/Desktop/TS5")
#Load functions getCT and extractCT
...
# Get data on study type, intervention type, 
#Phase, gender and age group of participants
alopeciaCT <- getCT(sTerm = "Androgenetic Alopecia", sStartEnd = "17/11/2021", outfile = "alopeciaCT.xml")
alopeciaCT <- extractCT(alopeciaCT)
write.csv(alopeciaCT, file = "alopeciaCT.csv")

A<-alopeciaCT$studyType
A<-as.data.frame(A, stringsAsFactors = default.stringsAsFactors())
B<-count(A)

write.csv(B, file = "studiesPerStudyType.csv")

alopeciaCT$interventionTypes
C<-alopeciaCT$interventionTypes
D<-as.data.frame(C, stringsAsFactors = default.stringsAsFactors())
D<-count(C)

write.csv(D, file = "studiesPerIntervType.csv", row.names = FALSE)

alopeciaCT$phases
E<-alopeciaCT$phases
G<-as.data.frame(E, stringsAsFactors = default.stringsAsFactors())
G<-count(E)

write.csv(G, file = "studiesPerPhase.csv", row.names = FALSE)

alopeciaCT$gender
H<-alopeciaCT$gender
I<-as.data.frame(H, stringsAsFactors = default.stringsAsFactors())
I<-count(H)

write.csv(I, file = "studiesPerGender.csv")

alopeciaCT$ageGroups
J<-alopeciaCT$ageGroups
K<-as.data.frame(J, stringsAsFactors = default.stringsAsFactors())
K<-count(J)

write.csv(K, file = "studiesPerAgeGroup.csv")
